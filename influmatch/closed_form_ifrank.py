# Closed-form last-layer influence-guided ranking consistency for FixMatch.
#
# IF(u,l) = scale * [(p_u-y_u) dot (p_l-y_l)] * [phi_u dot phi_l], where
# phi is the 512-dimensional feature entering the classifier. Influence scores
# are detached guidance; gradients flow through strong-view cosine relations.
import itertools
import torch
...
class ClosedFormIFRankMixin:
    ...
    def _hard_label_grad(self, logits, targets, apply_class_weights=True, apply_focal=True):
        probs = F.softmax(logits, dim=1)
        one_hot = F.one_hot(targets, self.num_classes).float()
        grad = probs - one_hot

        # Match the supervised criterion: weighted CE scales the per-example
        # last-layer logit gradient even when focal loss is disabled.
        class_weights = getattr(self.ce_loss, 'class_weights', None) if apply_class_weights else None
        alpha = logits.new_ones(targets.shape[0])
        if class_weights is not None:
            alpha = class_weights.to(logits.device).gather(0, targets)

        focal_gamma = float(getattr(self.ce_loss, 'focal_gamma', 0.0)) if apply_focal else 0.0
        if focal_gamma <= 0:
    ...
    def ifrank_loss(self, logits_x_lb, y_lb, logits_x_ulb_w, logits_x_ulb_s, phi_lb, phi_uw, phi_us):
        ...
        with torch.no_grad():
            ...
            # The supervised objective may use class/focal weighting, while
            # FixMatch's consistency CE is intentionally unweighted.
            g_l = self._hard_label_grad(lg_l, y_lb, apply_class_weights=True)
            ...
            if if_target == 'soft':
                ...
                g_uw = self._hard_label_grad(
                    lg_uw, y_u_idx, apply_class_weights=False, apply_focal=False)
                g_us = self._hard_label_grad(
                    lg_us, y_u_idx, apply_class_weights=False, apply_focal=False)
            if mean_reduce:
    ...
    def ifrank_arguments():
        return [
            ...
            # The closed-form IF branch is enabled unless an ablation explicitly
            # sets this scale to zero. A zero default silently removed IF.
